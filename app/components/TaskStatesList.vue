<template>
  <section class="rounded-xl border border-slate-200 bg-white p-6">
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h2 class="text-lg font-semibold text-slate-900">Статусы задач</h2>
        <p class="mt-1 text-sm text-slate-500">{{ items.length }} статусов</p>
      </div>
    ...
    <div v-if="showForm" class="mb-6 rounded-xl border border-slate-200 bg-slate-50 p-4">
      <div class="mb-4 flex items-center justify-between">
        <h3 class="text-sm font-semibold text-slate-900">{{ editingItem ? 'Редактировать' : 'Создать' }} статус</h3>
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div v-if="editingItem" class="flex items-center gap-2">
          ...
          />
          <label for="status-checkbox" class="text-sm font-medium text-slate-700">
            Активный
          </label>
        </div>
      </div>
    ...
    <div v-else-if="error" class="rounded-lg border border-rose-200 bg-rose-50 p-4">
      <p class="text-sm font-semibold text-rose-800">Не удалось загрузить статусы</p>
...
const items = ref<TaskState[]>([])
const loading = ref(false)
const error = ref<string | null>(null)

const showForm = ref(false)
const editingItem = ref<TaskState | null>(null)
...
function editItem(item: TaskState) {
  form.name = item.name
...
async function loadItems() {
  ...
  try {
    ...
    const result = await graphqlRequest<{ getTaskStates: TaskState[] }>(
      query,
      {},
      token.value
    )
    
    items.value = result.getTaskStates ?? []
  } catch (e: any) {
    error.value = e?.message || 'Не удалось загрузить статусы'
  } finally {
    loading.value = false
  }
}
...
async function submitForm() {
  ...
  try {
    if (editingItem.value) {
      // Update
      const mutation = `
        mutation UpdateTaskState($id: Int!, $taskStateUpdate: TaskStateUpdateInput!) {
          updateTaskState(id: $id, taskStateUpdate: $taskStateUpdate) {
            id
            name
            color
            status
      ...
      // Create
      const mutation = `
        mutation CreateTaskState($taskStateData: TaskStateCreateInput!) {
          createTaskState(taskStateData: $taskStateData) {
            id
            name
            color
            status
...
async function deleteItem(item: TaskState) {
